...
namespace InternalRealtimeCSG
{
	...
	{
		public static MeshInstanceKey GenerateKey(RenderSurfaceType renderSurfaceType, int subMeshIndex, Material renderMaterial, PhysicMaterial physicsMaterial)
		{
			var realRenderMaterial = renderMaterial;
			if (renderSurfaceType != RenderSurfaceType.Normal)
				realRenderMaterial = null;

			return new MeshInstanceKey(renderSurfaceType, subMeshIndex, (!realRenderMaterial ) ? 1 : realRenderMaterial.GetInstanceID(), 
																		(!physicsMaterial) ? 1 : physicsMaterial.GetInstanceID());
		}

		private MeshInstanceKey(RenderSurfaceType renderSurfaceType, int subMeshIndex, int renderMaterialInstanceID, int physicsMaterialInstanceID)
		{
		...
		public override int GetHashCode()
		{
			...
			return hash + (hash1 ^ hash2 ^ hash3 ^ hash4) + (hash1 + hash2 + hash3 + hash4) + (hash1 * hash2 * hash3 * hash4);
		}
	}
	...
	public sealed class GeneratedMeshInstance : MonoBehaviour
	{
		...
		{
			return MeshInstanceKey.GenerateKey(RenderSurfaceType, SubMeshIndex, RenderMaterial, PhysicsMaterial);
		}
		internal void Awake()
		{
