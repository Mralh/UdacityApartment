...
namespace RealtimeCSG
{
	internal abstract class GeneratorBase : ScriptableObject//, IBrushGenerator
	{
		...
		protected void SetBrushTransformation(Transform transform)
		{
			...
			{
				transform.rotation = brushRotation.Value *
		...
		protected void CalculateWorldSpaceTangents()
		{
			...
			{
				hadForcedGrid = Grid.ForceGrid;
			}
			if (hadForcedGrid)
			{
				Grid.ForceGrid = false;
				Grid.UpdateGridOrientation();
			}
			...
			gridNormal   = Grid.VectorFromGridSpace(gridSpaceNormal);
		...
		internal void UnregisterAllMaterials()
		{
			for (int i = 0; i < generatedBrushes.Length; i++)
			{
				...
				if (brush.Shape != null && brush.gameObject.activeSelf)
					InternalCSGModelManager.UnregisterMaterials(parentModel, brush.Shape, false);
			}
		}
		
		internal void RegisterAllMaterials()
		{
			for (int i = 0; i < generatedBrushes.Length; i++)
			{
				var brush = generatedBrushes[i];
