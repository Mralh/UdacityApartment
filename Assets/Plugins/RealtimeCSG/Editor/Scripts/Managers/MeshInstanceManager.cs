...
namespace InternalRealtimeCSG
{
	internal static class MeshInstanceManager
	{
		...
		public static GeneratedMeshInstance FindMesh(GeneratedMeshes container, int subMeshIndex, Material renderMaterial, PhysicMaterial physicsMaterial)
		{
			var renderSurfaceType = GetRenderSurfaceType(container.owner, renderMaterial);

			var key	= MeshInstanceKey.GenerateKey(renderSurfaceType, subMeshIndex, renderMaterial, physicsMaterial);
		...
		public static GeneratedMeshInstance GetMesh(GeneratedMeshes container, int subMeshIndex, Material renderMaterial, PhysicMaterial physicsMaterial)
		{
			if (container.meshInstances == null ||
				container.meshInstances.Count == 0)
			{ 
