	...
	internal static class InternalCSGModelManager
	{
		...
		public static void EnsureInitialized(CSGBrush component)
		{
			...
			if (component.Shape.CutNodes == null ||
				...
				component.Shape.CutNodes.Length != component.Shape.Surfaces.Length || 
		...
		static void RegisterBrush(CSGBrush brush)
		{
			...
			} else
			{
				...
				if ((brush.flags & BrushFlags.InfiniteBrush) != BrushFlags.InfiniteBrush)
				{
					...
					{
						ShapeUtility.CreateCutter(brush.Shape, brush.ControlMesh);
		...
		public static void CheckSurfaceModifications(CSGBrush brush, bool surfacesModified = false)
		{
			...
			if (!updateTexGens)
			{
				...
				if (!updateTexGens)
				{
					for (int i = 0; i < shape.TexGens.Length; i++)
					{
						if (shape.TexGens[i].RenderMaterialIndex == compareShape.prevTexGens[i].RenderMaterialIndex)
							continue;
		...
		public static void OnHierarchyModified()
		{
			...
			if (External.SetModelChildren != null)
			{
				for (var i = 0; i < Models.Length; i++)
				{
					...
					var childList = UpdateChildList(itemCache.ParentData);
//					Debug.Log("SetModelChildren[C#] modelID: " + item.modelID + " nodeID: " + item.nodeID + " count: " + childList.Length + " name: " + item.name, item);
		...
		public static void RegisterMaterials(CSGModel model, Shape shape, bool notifyExternal = true)
		{
			...
			for (var i = 0; i < shape.TexGens.Length; i++)
			{
				...
				shape.TexGens[i].RenderMaterialIndex = RegisterMaterial(model, currentMaterial);
			}
		...
		public static void UnregisterMaterials(CSGModel model, Shape shape, bool notifyExternal = true)
		{
			...
			for (var i = 0; i < shape.TexGens.Length; i++)
			{
				...
				shape.TexGens[i].RenderMaterialIndex = -1;
			}
		...
		private static void GetModelMesh(GeneratedMeshes meshContainer, int modelID, ref Matrix4x4 modelMatrix, VertexChannelFlags vertexChannelFlags, MeshType meshType, int meshUniqueID, Material material, PhysicMaterial defaultPhysicsMaterial, bool vertexLayoutModified, HashSet<GeneratedMeshInstance> foundInstances)
		{
			...
			var vertexHashValues		= new UInt64[subMeshCount];
			var triangleHashValues		= new UInt64[subMeshCount];
			var surfaceHashValues		= new UInt64[subMeshCount];
			var vertexCounts			= new Int32[subMeshCount];
			var	indexCounts				= new Int32[subMeshCount];
			...
			{ 
				...
				var meshInstance = MeshInstanceManager.GetMesh(meshContainer, subMeshIndex, material, defaultPhysicsMaterial);
		...
		public static bool UpdateMeshes(bool forceUpdate = false)
		{
			...
			try
			{
				...
				for (var i = 0; i < modelCount; i++)
				{
					...
					if (!External.HasModelChanged(modelID))
						continue;
					...
					// remove all mesh instances that have no corresponding brush materials
					...
					for (var materialIndex = 0; materialIndex < materialCount; materialIndex++)
					{
						...
						GetModelMesh(meshContainer, modelID, ref modelMatrix, vertexChannelFlags, MeshType.Renderable, materialIndex, material, null, vertexLayoutModified, foundInstances);
					}

					if (renderSurfaceType == RenderSurfaceType.ShadowOnly ||
						((model.HaveCollider || model.IsTrigger) && renderSurfaceType == RenderSurfaceType.Normal) ||
					...
					if (renderSurfaceType == RenderSurfaceType.Normal)
					{
						GetModelMesh(meshContainer, modelID, ref modelMatrix, VertexChannelFlags.UV0, MeshType.Invisible, 0, invisibleMaterial, null, vertexLayoutModified, foundInstances);
					}
